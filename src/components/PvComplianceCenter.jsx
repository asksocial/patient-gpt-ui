...
const PV_LIFECYCLE_TOOLTIPS = {
  ...
  not_relevant: "The reviewer determined that the content is not reportable and does not require PV escalation. The record and rationale remain retained.",
...
const PV_REVIEW_FIELD_TOOLTIPS = {
  "Does the content mention or reasonably imply a sponsor product?": "Confirm whether the verbatim names or reasonably implies the implicated product or procedure. This is one of the four minimum ICSR criteria.",
  ...
  "Reviewer rationale": "Document the evidence and reasoning supporting the reportability or non-reportability decision.",
...
function label(value) {
  if (value === "not_relevant") return "Not Reportable";
  if (value === "close_not_relevant") return "Close Not Reportable";
...
export default function PvComplianceCenter({ initialTab = "overview", initialMessage = "", therapeuticArea = "", workspaceId = "", workspaces = [], onRefreshWorkspaces, onNavigate, onInitialMessageConsumed }) {
  ...
  const [sponsorEmailDeliveryConfigured, setSponsorEmailDeliveryConfigured] = useState(false);
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setSponsorEmailDeliveryConfigured(Boolean(payloads[8].emailDelivery?.configured));
    } catch (loadError) {
...
function Overview({ metrics, statusCounts, onSelectLifecycle, onNavigate }) {
  return <div className="space-y-5">
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Metric label="Human review completed" value={metrics.reviewedRecords ?? 0} detail="Escalated or closed records" tooltip="Unique records with a retained human-review decision, including records escalated for sponsor transfer and records closed as not reportable." />
  ...
  return <div className="space-y-5">
    ...
    <Card title="Health Experience Detection" subtitle="Broader product-linked medical experiences and special situations are retained here separately from potential AE/ADR records in the Review Queue.">
      <div className="rounded-xl border border-cyan-300/15 bg-cyan-300/[0.05] p-4 text-xs leading-5 text-cyan-100/65">
        This segment includes medication errors, overdose, misuse or abuse, pregnancy exposure, lack of efficacy, product-quality complaints, and other safety-significant observations. Slang, misspellings, and variable patient language are normalized through the configured detection concepts. These records are safety signals for surveillance—not confirmed adverse reactions.
      </div>
      ...
      </> : <div className="mt-4"><Empty>{query ? "No health experience detections match this search." : "No health-experience-only records were detected for this topic."}</Empty></div>}
    </Card>
  </div>;
}
...
function RecordWorkbench({ detail, busy, onMutate, onRefresh }) {
  ...
  async function review(decision, includeStructuredAssessment = markedReportable) {
    if (!rationale.trim()) {
  ...
  return <div className="space-y-5">
    ...
    {!markedReportable && !["not_relevant", "health_experience", "transferred", "acknowledged", "reconciled"].includes(record.status) ? (
      <Card title="Initial reportability decision" subtitle="Determine whether the mention contains a potentially reportable AE/ADR before opening the complete ontology and ICH case assessment.">
        {reclassifyingHealth ? <div className="space-y-4">
          <div className="rounded-xl border border-cyan-300/20 bg-cyan-300/[0.05] p-4"><p className="text-sm font-medium text-cyan-100/80">Reclassify as Health Experience</p><p className="mt-1 text-xs leading-5 text-white/45">Use this outcome when the mention is product-linked and safety-significant but does not contain an AE/ADR signal. The original automated detection and prior review remain preserved in History &amp; Audit.</p></div>
          ...
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => { setMarkedReportable(true); setReviewError(""); }} className="cursor-pointer rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-black">Mark as Reportable</button><button type="button" onClick={() => { setSelectedClasses((current) => current.filter((classification) => classification !== "adverse_event")); setReclassifyingHealth(true); setReviewError(""); }} className="cursor-pointer rounded-xl border border-cyan-300/25 px-4 py-2.5 text-sm font-medium text-cyan-200">Reclassify as Health Experience</button><button type="button" disabled={busy.startsWith("review:")} onClick={() => review("close_not_relevant", false)} className="cursor-pointer rounded-xl border border-white/10 px-4 py-2.5 text-sm text-white/60 disabled:cursor-not-allowed disabled:opacity-40">{busy === "review:close_not_relevant" ? "Saving…" : "Close as Not Reportable"}</button></div>
        </div>}
        {reviewError ? <p role="alert" className="mt-3 text-xs text-amber-300">{reviewError}</p> : null}
      </Card>
    ) : null}
    {markedReportable && !["transferred", "acknowledged", "reconciled"].includes(record.status) ? (
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button type="button" disabled={busy.startsWith("review:")} onClick={() => review("close_not_relevant")} className="cursor-pointer rounded-xl border border-white/10 px-4 py-2.5 text-sm text-white/60 disabled:cursor-not-allowed disabled:opacity-40">{busy === "review:close_not_relevant" ? "Saving…" : "Close as Not Reportable"}</button>
            </div>
          </div>
        </div>
      </Card>
...
function ReviewerWorkflowGuide() {
  const steps = [
    ...
    ["Reportability", "Choose Mark as Reportable for a potential AE/ADR, Reclassify as Health Experience for a non-AE safety observation, or close the mention as not reportable."],
...
function SponsorHandoff({ therapeuticArea, sponsorCases, qaNotRelevantCases, emailDeliveryConfigured, busy, onMutate, onOpenRecord }) {
  ...
  async function shareSponsorReport() {
    ...
    download.click();
    download.remove();
    const subject = encodeURIComponent(`AskSocial PV sponsor screening report - ${therapeuticArea || "All topics"}`);
  ...
  return <div className="space-y-5">
    ...
    <QaNotRelevantHandoff therapeuticArea={therapeuticArea} cases={qaNotRelevantCases} emailDeliveryConfigured={emailDeliveryConfigured} busy={busy} onMutate={onMutate} />
  </div>;
}

function QaNotRelevantHandoff({ therapeuticArea, cases, emailDeliveryConfigured, busy, onMutate }) {
  ...
  async function shareQaReport() {
    ...
    const data = await onMutate(
      ...
      "QA-only Not Reportable export activity recorded. No PV lifecycle status or Day Zero timestamp was changed."
    );
    setConfirmOpen(false);
    if (!data) return;
    setDeliveryNotice(data.message || (data.delivery === "provider" ? "QA email sent." : "No email was sent. The PDF was prepared for a manual email draft."));
    if (data.delivery === "provider") return;
    const download = document.createElement("a");
    download.href = exportHref;
    download.download = data.fileName || "asksocial-pv-qa-not-reportable-export-test.pdf";
    document.body.appendChild(download);
    download.click();
    download.remove();
    const subject = encodeURIComponent(`[QA TEST - NOT FOR SUBMISSION] AskSocial PV export - ${therapeuticArea || "All topics"}`);
    const body = encodeURIComponent(`QA TEST ONLY - NOT FOR SPONSOR SUBMISSION OR REGULATORY REPORTING.\n\nThe attached AskSocial PDF contains ${data.caseCount} mention${data.caseCount === 1 ? "" : "s"} closed as Not Reportable and is being used only to validate export and handoff mechanics. The PDF has been downloaded to this device; attach it before sending.`);
    window.location.href = `mailto:${encodeURIComponent(email)}?subject=${subject}&body=${body}`;
  }

  return <Card title="QA export test · Not Reportable mentions" subtitle="Use recently closed records to validate PDF rendering and email mechanics when no four-criteria ICSR case is available. QA activity is segregated from sponsor submission and never changes lifecycle status or starts Day Zero." actions={<ToneBadge tone="approaching">QA TEST ONLY</ToneBadge>}>
    {confirmOpen ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" role="dialog" aria-modal="true" aria-label="Confirm Not Reportable QA handoff"><div className="w-full max-w-lg rounded-3xl border border-rose-400/25 bg-[#080808] p-6 shadow-2xl"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-rose-300">QA test · not for submission</p><h2 className="mt-2 text-xl font-semibold text-white">Share {cases.length} Not Reportable QA example{cases.length === 1 ? "" : "s"}?</h2><p className="mt-3 text-sm leading-6 text-white/45">{emailDeliveryConfigured ? <>AskSocial will email a prominently labeled QA PDF to <span className="font-medium text-white/75">{recipientEmail.trim()}</span>.</> : <>Direct email is not configured. AskSocial will download the QA PDF and attempt to open a local email draft addressed to <span className="font-medium text-white/75">{recipientEmail.trim()}</span>; you must attach the PDF and send the draft yourself.</>} It will not create a sponsor transfer, change record status, or start Day Zero.</p><div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setConfirmOpen(false)} disabled={busy === "sponsor-report:qa-share"} className="cursor-pointer rounded-xl border border-white/10 px-4 py-2.5 text-sm text-white/60 disabled:opacity-40">Cancel</button><button type="button" onClick={shareQaReport} disabled={busy === "sponsor-report:qa-share"} className="cursor-pointer rounded-xl bg-rose-100 px-4 py-2.5 text-sm font-semibold text-rose-950 disabled:opacity-40">{busy === "sponsor-report:qa-share" ? "Preparing…" : emailDeliveryConfigured ? "Send QA email" : "Download PDF & open draft"}</button></div></div></div> : null}
    {cases.length ? <div className="space-y-4">
      <div className="rounded-xl border border-rose-400/20 bg-rose-400/[0.06] px-4 py-3 text-xs leading-5 text-rose-100/75"><strong>Non-reportable test population:</strong> the {cases.length} most recent permitted mentions closed as Not Reportable. The PDF and email are marked QA TEST ONLY and must be sent only to an internal or controlled test mailbox.</div>
      {!emailDeliveryConfigured ? <div className="rounded-xl border border-amber-400/20 bg-amber-400/[0.06] px-4 py-3 text-xs leading-5 text-amber-100/75"><strong>Direct email is not configured:</strong> AskSocial cannot send this package from staging. Sharing will download the PDF and attempt to open an email draft; you must attach the file and send it manually.</div> : null}
      {deliveryNotice ? <div role="status" className={`rounded-xl border px-4 py-3 text-xs leading-5 ${emailDeliveryConfigured ? "border-cyan-400/20 bg-cyan-400/[0.06] text-cyan-100/75" : "border-amber-400/20 bg-amber-400/[0.06] text-amber-100/75"}`}>{deliveryNotice}</div> : null}
      ...
      <div className="grid gap-3 rounded-xl border border-rose-400/15 bg-rose-400/[0.04] p-4 lg:grid-cols-[auto_1fr_auto] lg:items-end">
        ...
        <button type="button" onClick={() => { setDeliveryNotice(""); setConfirmOpen(true); }} disabled={!recipientEmail.trim() || !testRecipientConfirmed || busy === "sponsor-report:qa-share"} className="cursor-pointer rounded-xl border border-rose-300/30 px-4 py-2.5 text-sm font-semibold text-rose-200 disabled:cursor-not-allowed disabled:opacity-40">Share QA handoff</button>
      </div>
    ...
    </div> : <Empty>No Not Reportable reviews are available for QA export in the selected topic.</Empty>}
  </Card>;
}
