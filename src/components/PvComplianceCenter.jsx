...
function ScreeningStatus({ therapeuticArea, bundledCorpus, sources, screenings, imports, libraries, busy, setBusy, setMessage, onReload, onMutate }) {
  ...
  const [csvForm, setCsvForm] = useState({ libraryId: "", sourceId: "", dataOrigin: "curated", dateColumn: "", contentColumns: "", sourceUrlColumn: "", externalIdColumn: "" });
  ...
  function csvField(key) { return { value: csvForm[key], onChange: (event) => setCsvForm((current) => ({ ...current, [key]: event.target.value })) }; }
  async function uploadCsv(event) {
    event.preventDefault();
    if (!csvFile || !csvForm.libraryId) return;
    setBusy("csv-import"); setMessage("");
    const formData = new FormData();
    formData.set("file", csvFile);
    Object.entries(csvForm).forEach(([key, value]) => formData.set(key, value));
    try {
      const response = await fetch("/api/pv/imports", { method: "POST", body: formData });
      const data = await response.json();
      if (!response.ok || !data.ok) { setMessage(data.error || "CSV screening failed."); return; }
      setMessage(`CSV made available at ${formatDate(data.import.available_at)}. ${data.import.screened_count} rows screened; ${data.import.routed_count} potential records routed.`);
      setCsvFile(null); setUploadKey((value) => value + 1);
      await onReload();
    } catch (uploadError) {
      setMessage(uploadError instanceof Error ? uploadError.message : "CSV screening failed.");
    } finally {
      setBusy("");
    }
  }
  ...
  return <div className="space-y-5">
    ...
    <Card title="CSV social-data intake" subtitle="Two timestamps are retained: the original post date from the mapped CSV date column and the server-recorded reviewer-identification date when the dataset becomes available to authorized AskSocial users in this tenant.">
      <form onSubmit={uploadCsv} className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><label className="text-xs text-white/40">Post-date column<input {...csvField("dateColumn")} placeholder="Auto-detect Date, Post Date, etc." className="mt-1.5 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white" /></label><label className="text-xs text-white/40">Content columns<input {...csvField("contentColumns")} placeholder="Auto-detect, or Text, Title" className="mt-1.5 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white" /></label><label className="text-xs text-white/40">Source URL column<input {...csvField("sourceUrlColumn")} placeholder="Auto-detect" className="mt-1.5 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white" /></label><label className="text-xs text-white/40">External ID column<input {...csvField("externalIdColumn")} placeholder="Auto-detect" className="mt-1.5 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2.5 text-sm text-white" /></label></div>
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-cyan-400/15 bg-cyan-400/[0.04] p-3"><div><p className="text-xs font-medium text-cyan-100/70">Governing clock for CSV imports: reviewer identification</p><p className="mt-1 text-[11px] leading-5 text-white/35">The browser cannot supply or alter this timestamp. AskSocial records it on the server when the upload is registered and preserves the CSV post date separately.</p></div><button disabled={!csvFile || !csvForm.libraryId || busy === "csv-import"} className="rounded-xl bg-white px-4 py-2.5 text-sm font-medium text-black disabled:opacity-40">{busy === "csv-import" ? "Screening CSV…" : "Upload & screen CSV"}</button></div>
      </form>
      <div className="mt-5 space-y-2">{imports.length ? imports.slice(0, 12).map((batch) => <div key={batch.id} className="rounded-xl border border-white/10 bg-black/30 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-white/70">{batch.file_name}</p><p className="mt-1 text-xs text-white/35">Reviewer identification / tenant availability: {formatDate(batch.available_at)}</p><p className="mt-1 text-[11px] text-white/25">Post date from CSV column “{batch.date_column}” · {batch.screened_count}/{batch.row_count} screened · {batch.routed_count} routed · {batch.duplicate_count} duplicates · {batch.failed_count} failed</p></div><div className="flex flex-wrap gap-2"><ToneBadge tone={batch.status === "completed" ? "healthy" : batch.status === "failed" ? "breached" : "approaching"}>{label(batch.status)}</ToneBadge><ToneBadge tone="complete">Day zero: reviewer identification</ToneBadge></div></div></div>) : <Empty>No CSV social-data batches have been made available.</Empty>}</div>
    </Card>
