...
function ReviewQueue({ records, selected, busy, onOpen, onMutate, onRefreshRecord }) {
  ...
  </Card>{selected ? <RecordWorkbench detail={selected} busy={busy} onMutate={onMutate} onRefresh={() => onRefreshRecord(selected.record.id)} /> : null}</div>;
}
...
function RecordWorkbench({ detail, busy, onMutate, onRefresh }) {
  ...
  async function review(decision) {
    const data = await onMutate(`/api/pv/records/${record.id}`, { method: "PATCH", payload: { action: "review", productMention, healthExperience, classifications: selectedClasses, rationale, decision } }, `review:${decision}`, decision === "escalate" ? "Record marked ready for sponsor transfer." : "Record retained and closed as not PV relevant.");
    if (data) onRefresh();
  }
  ...
  return <div className="space-y-5">
    <div className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <Card title={`PV Record ${record.id.slice(0, 8)}`} subtitle="Original evidence is immutable. Translations, review decisions, and workflow events are stored separately.">
        ...
        <div className="grid gap-3 md:grid-cols-3"><Metric label="PV detection score" value={`${record.detection_score}/100`} /><Metric label="Product confidence" value={`${record.product_confidence}%`} /><Metric label="Health experience" value={`${record.health_experience_confidence}%`} /></div>
...
function Configuration({ libraries, concepts, busy, onMutate }) {
  const [libraryForm, setLibraryForm] = useState({ name: "", sponsorName: "", productId: "", market: "US", language: "en", detectionThreshold: 55 });
