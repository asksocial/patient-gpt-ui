...
assert(resolvePvReviewCompletionNavigation("reclassify_health_experience")?.destination === "pv_overview", "Health Experience reclassification must return to Compliance Overview so the affected lifecycle count is visible.");
assert(resolvePvReviewCompletionNavigation("escalate")?.destination === "pv_overview", "Completed reportability assessments must return to Compliance Overview so the affected lifecycle count is visible.");
assert(resolvePvReviewCompletionNavigation("draft") === null, "Non-terminal review decisions must remain in the structured-review flow.");
assert(workbench.includes("if (options.completionNavigation)") && workbench.includes("onNavigate?.(options.completionNavigation.destination, { notice: success })"), "The successful terminal mutation itself must navigate with a retained confirmation so the saved decision and visible destination cannot diverge.");
assert(workbench.includes("options.onError?.(failureMessage)") && workbench.includes("onError: setReviewError"), "A failed terminal PV mutation must display the backend reason inside the structured-review panel instead of failing without visible feedback.");
assert(workbench.includes("if (data.record?.id)") && !workbench.slice(workbench.indexOf("if (options.completionNavigation)"), workbench.indexOf("if (options.refresh !== false)")).includes("await loadAll();"), "Terminal PV decisions must merge the verified saved record and navigate immediately instead of waiting on an unrelated broad refresh.");
assert(workbench.includes("{ refresh: !completionNavigation, completionNavigation, onError: setReviewError }") && workbench.includes("if (data && !completionNavigation) onRefresh()"), "The structured-review caller must delegate terminal refresh, navigation, and inline failure feedback to the saved mutation.");
assert(workbench.includes('role="status" aria-live="polite"') && workbench.includes('initialMessage = ""') && workbench.includes("onInitialMessageConsumed?.()"), "Terminal PV decisions must expose an accessible saved-state confirmation on the destination screen.");
assert(workspaceShellSource.includes("pvNavigationNotice") && workspaceShellSource.includes("initialMessage={pvNavigationNotice}") && workspaceShellSource.includes('options.notice || ""'), "Workspace navigation must carry the one-time PV completion confirmation across the remounted destination.");
assert(workbench.includes('id="pv-lifecycle-ledger"') && workbench.includes('document.getElementById("pv-lifecycle-ledger")?.focus'), "Selecting a lifecycle count must move focus to the corresponding governed mention ledger.");
