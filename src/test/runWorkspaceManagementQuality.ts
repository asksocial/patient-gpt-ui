...
assert(manager.includes('const selectedWorkspaceId = selected?.id || ""') && manager.includes("if (!selectedWorkspaceId)"), "Workspace management must safely render and operate when the user has no selected workspace.");
assert(shell.includes('data-testid="active-workspace-indicator"') && shell.includes('aria-label="Select current workspace"') && shell.includes("Save current session as workspace") && shell.includes("activeWorkspace.name"), "The header must offer session-to-workspace saving until a workspace is selected, then display its name.");
assert(shell.includes('data-testid="active-module-indicator"') && shell.includes('activeModule?.name || "None"') && shell.includes('Module: {activeModule?.name || "None"}'), "The module indicator must show None until a module is selected, then retain the selected module beside the workspace control.");
assert(!shell.includes('window.localStorage.getItem(\n                  "asksocial.activeModule"') && !shell.includes('window.localStorage.setItem(\n          "asksocial.activeModule"'), "Module selection must not be inferred or restored before the user explicitly selects a module in the current experience.");
assert(shell.includes("saveCurrentSessionAsWorkspace") && shell.includes('method: "POST"') && shell.includes("await selectWorkspace(data.workspace.id)"), "The header action must create, select, and attach the current session to its new workspace.");
...
assert(shell.includes("!workspace.archivedAt") && shell.includes('disabled={workspace.role === "viewer"}'), "The selected-workspace dropdown must list only active associated workspaces and prevent selection of view-only destinations.");
