  ...
  const groupedSources = useMemo(() => {
    ...
    for (const source of sources) {
      if (source?.block_type === "insight") {
        ...
        voices.push(source);
      } else if (source?.block_type === "recommendation") {
    ...
    return { insights, voices, recommendations, other };
  }, [sources]);
    ...
    <main className="min-h-screen bg-black text-white flex">
      <aside className="w-[280px] border-r border-white/10 p-4 flex flex-col gap-6">
        ...
        <div className="text-xs text-white/40 leading-5">
          Insight blocks summarize patterns across the conversation.
          Representative voice blocks show real patient or caregiver expressions.
          Recommendation blocks capture action-oriented implications from the
          report.
        </div>
      </aside>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="max-w-5xl mx-auto">
              ...
              {loading ? (
                ...
                <div className="text-white/50">
                  ...
                  <div className="mt-2 text-white/80">
                    “Where do patients typically go online to discuss Hep B?”
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="mt-8">
              ...
              {!!groupedSources.recommendations.length && (
                <SourceSection
                  title="Recommendations"
                  subtitle="Action-oriented implications from the source material"
                >
                  {groupedSources.recommendations.map((s, idx) => (
                    <RecommendationCard
                      key={s?.id ?? `recommendation-${idx}`}
                      source={s}
                    />
                  ))}
                </SourceSection>
...
function RecommendationCard({ source }) {
  return (
